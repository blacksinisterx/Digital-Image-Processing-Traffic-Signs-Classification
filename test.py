...
# Create a DataFrame from the predicted_classes list
predicted_df = pd.DataFrame({'predicted_class': predicted_classes})

# Save the DataFrame to a CSV file
predicted_df.to_csv("test_predictions.csv", index=False)
(test_df['ClassId'] == predicted_classes).sum() / len(test_df) * 100
